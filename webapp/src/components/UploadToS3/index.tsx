import { getS3UploadUrl, getS3UploadName } from '@forum_project/shared/src/s3'
          ...
          void (async () => {
            ...
            try {
              if (files?.length) {
                ...
                // Генерация полного URL для документа
                const fullUrl = `https://s3.eu-north-1.amazonaws.com/forum-project/${s3Key}`
                
                // Устанавливаем полный URL в значение формы
                void formik.setFieldValue(name, fullUrl)
              }
            } catch (err: any) {
              console.error(err)
      ...
      <div className={css.buttons}>
        <Buttons>
          <Button
            ...
            color="green"
          >
            {value ? 'Загрузить другой' : 'Загрузить документ'}
          </Button>
          {!!value && !loading && (
            <Button
              type="button"
