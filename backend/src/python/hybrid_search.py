...
class HybridSearchProcessor:
    def _init_(self, embedding_model, index_name="med-search"):
...
class HybridSearchProcessorWithAnswerGeneration(HybridSearchProcessor):
    def _init_(self, embedding_model, index_name="med-search", llm_model=None):
        super()._init_(embedding_model, index_name)
    ...
    def generate_answer(self, query, search_results):
        ...
        # Сгенерировать ответ с учетом всей истории
...
search_processor_with_answers = HybridSearchProcessorWithAnswerGeneration(embedding_model=embedding_model, index_name="med-search", llm_model=llm)

search_processor_with_answers.initialize(training_texts)
search_processor_with_answers.process_texts(training_texts)

# Пример запросов
query1 = "Средство от головной боли"
answer1 = search_processor_with_answers.search_and_answer(query1)
